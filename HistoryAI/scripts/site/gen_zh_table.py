...
得出与 Python 相反的「长度变了」结论。所以 JS 侧必须数码位（`[...s].length`），
本脚本生成的表也照抄系统对扩展 B 的原始输出（哪怕是残缺代理码位），不做任何修补
—— Python 那边就是这样，JS 要一模一样。
"""
...
# 逐字枚举的区段：只收 BMP 内的汉字。
#
# 为什么不整段枚举星形平面（扩展 B 及以上）：系统对星形汉字一律返回残缺代理码位，
# 整段收下来等于往表里塞 7 万条垃圾，表从 150KB 涨到 3.7MB——比演示数据还大，
# 而用户根本不会在搜索框里输入扩展 B 汉字。星形字符改由「语料用字」那一遍补：
# 语料里真实出现的星形字符只有 51 个（扩展 B 46 + 扩展 E 2 + 私有使用区 3），
# 逐个收进表里就够正文转换用了。
#
 ...
 *
 * 注意：本表**照抄**系统输出，包括系统对 CJK 扩展 B 汉字返回的残缺代理码位。
 * 那不是本表引入的问题，Python 侧 search/zh.py 拿到的是同样的值；两边必须一样，
